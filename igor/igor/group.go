...
type GroupConfig struct {
	...
	// Verbose specifies that groups failing consistency checks are
	// completely described before being discarded.
	Verbose bool
}
...
func Group(clust [][]*pals.Pile, cfg GroupConfig) []graph.Nodes {
	...
	for p, n := range g.poisoned {
		if n > 1 {
			log.Printf("removing poisoned node: %s:%d-%d = %d",
				p.Loc.Name(), p.From, p.To, n,
			)
			g.delete(p)
		} else {
			log.Printf("warn possible poisoned node: %s:%d-%d",
				p.Loc.Name(), p.From, p.To,
			)
		}
	}
		...
		for _, n := range cc[i] {
			for _, e := range n.Edges() {
				if e.(*twistEdge).conflict {
					log.Printf("igor: discarding inconsistent connected component with %d nodes\n", len(cc[i]))

					if cfg.Verbose {
						graph.NewDepthFirst().Search(e.Head(),
							func(e graph.Edge) bool {
								ce := e.(*twistEdge)
								if ce.conflict {
									h := e.Head().(*pals.Pile)
									t := e.Tail().(*pals.Pile)
									fmt.Fprintf(os.Stderr, "conflict: %s[%d,%d)%v ~ %v ~ %s[%d,%d)%v\n",
										t.Loc.Name(), t.From, t.To, t.Strand,
										ce.twist,
										h.Loc.Name(), h.From, h.To, h.Strand,
									)
								}
								return true
							},
							func(n graph.Node) bool {
								...
								fmt.Fprintf(os.Stderr, "%s[%d,%d)%v\n",
									p.Loc.Name(), p.From, p.To, p.Strand)
								return false
							},
							nil,
						)
...
func (g pileGraph) connect(pile, partner *pals.Pile, twist seq.Strand) error {
	...
	if ok && e.twist != twist {
		if e.twist != seq.None {
			g.poisoned[pile]++
			g.poisoned[partner]++
			log.Printf("dropped edge between %s:%d-%d and %s:%d-%d",
				pile.Loc.Name(), pile.From, pile.To,
				partner.Loc.Name(), partner.From, partner.To,
			)
		}
		e.twist = seq.None
		return nil
